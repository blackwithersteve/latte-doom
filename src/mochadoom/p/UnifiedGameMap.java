...
public abstract class UnifiedGameMap implements ThinkerList {
    ...
    public class Specials {
        ...
        private static String boomStr(byte[] b, int off) {
            int len = 0;
            while (len < 8 && b[off + len] != 0) {
                len++;
            }
        ...
        public void InitPicAnims() {
            ...
            animdef_t[] defs = animdefs;
            try {
                if (DOOM.wadLoader.CheckNumForName("ANIMATED") != -1) {
                    final byte[] lump = DOOM.wadLoader.CacheLumpNameAsRawBytes("ANIMATED", 0);
                    final java.util.List<animdef_t> parsed = new java.util.ArrayList<>();
                    for (int o = 0; o + 23 <= lump.length; o += 23) {
                        if (lump[o] == -1) {
                            break;
                        }
                        final String end = boomStr(lump, o + 1);
                        final String start = boomStr(lump, o + 10);
                        final int speed = (lump[o + 19] & 0xFF) | ((lump[o + 20] & 0xFF) << 8)
                            | ((lump[o + 21] & 0xFF) << 16) | ((lump[o + 22] & 0xFF) << 24);
                        parsed.add(new animdef_t((lump[o] & 1) != 0, end, start,
                            Math.max(1, speed)));
                    }
                    if (!parsed.isEmpty()) {
                        parsed.add(new animdef_t(false, "", "", 0)); // terminator slot
                        defs = parsed.toArray(new animdef_t[0]);
                        System.out.println("[lattedoom-boom] ANIMATED lump: "
                            + (parsed.size() - 1) + " animations");
                    }
                }
            } catch (Exception e) {
                System.err.println("[lattedoom-boom] ANIMATED parse failed: " + e);
            }
            if (defs.length > anims.length) {
                anims = new anim_t[defs.length + 4];
            }
            final animdef_t[] animdefs = defs; // shadow: the loop below reads the chosen set
    ...
    public class Switches {
        ...
        public void InitSwitchList() {
            ...
            try {
                if (DOOM.wadLoader.CheckNumForName("SWITCHES") != -1) {
                    final byte[] lump = DOOM.wadLoader.CacheLumpNameAsRawBytes("SWITCHES", 0);
                    final java.util.List<switchlist_t> parsed = new java.util.ArrayList<>();
                    for (int o = 0; o + 20 <= lump.length; o += 20) {
                        final int ep = (lump[o + 18] & 0xFF) | ((lump[o + 19] & 0xFF) << 8);
                        if (ep == 0) {
                            break;
                        }
                        parsed.add(new switchlist_t(boomStr(lump, o), boomStr(lump, o + 9),
                            (short) ep));
                    }
                    if (!parsed.isEmpty()) {
                        parsed.add(new switchlist_t("", "", (short) 0)); // terminator
                        list = parsed.toArray(new switchlist_t[0]);
                        System.out.println("[lattedoom-boom] SWITCHES lump: "
                            + (parsed.size() - 1) + " pairs");
                    }
                }
            } catch (Exception e) {
                System.err.println("[lattedoom-boom] SWITCHES parse failed: " + e);
            }
            final switchlist_t[] alphSwitchList = list; // shadow: loop reads the chosen set
        ...
        public final void StartButton(line_t line, bwhere_e w, int texture, int time) {
            ...
            if (Engine.getConfig().equals(Settings.extend_button_slots_limit, Boolean.TRUE)) {
                ...
                throw new mochadoom.DoomFatalError("P_StartButton: no button slots left!");
            }
        }
