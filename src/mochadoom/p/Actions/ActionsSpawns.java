...
public interface ActionsSpawns extends ActionsSectors {
     ...
     */
    default mobj_t SpawnMobjNum(@fixed_t int x, @fixed_t int y, @fixed_t int z, int typenum) {
        mobj_t mobj;
        state_t st;
        mobjinfo_t info;
        ...
        info = mobjinfo[typenum];

        mobj.type = typenum < mobjtype_t.NUMMOBJTYPES.ordinal()
            ? mobjtype_t.values()[typenum] : mobjtype_t.NUMMOBJTYPES;
        mobj.mobj_typenum = typenum;
        mobj.info = info;
        mobj.x = x;
        mobj.y = y;
    ...
    default mobj_t SpawnMapThing(mapthing_t mthing) {
        ...
        if (i == mobjinfo.length) { // DEH may have extended the table past NUMMOBJTYPES
            ...
            if (mthing.type == 5001 || mthing.type == 5002) {
                contextRequire(ActionsBoom.KEY_BOOM_PUSH_POINTS).points
                    .add(new int[] { mthing.x, mthing.y, mthing.type });
                return null;
            }
