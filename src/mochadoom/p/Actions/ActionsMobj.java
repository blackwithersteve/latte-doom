...
public interface ActionsMobj extends ActionsThings, ActionsMovement, ActionsTeleportation {
    ...
    default void DamageMobj(mobj_t target, mobj_t inflictor, mobj_t source, int damage) {
        ...
        if ((inflictor != null)
            ...
            if (player != null) {
                ...
                mochadoom.Engine.recordPlayerThrust(player,
                    Math.cos(a), Math.sin(a), thrust / 65536.0);
            }
        }
        ...
        if (player != null) {
            ...
            // tic pipeline wedged (the death-freeze). Pain/thrust still apply.
            if (mochadoom.Engine.PLAYERS_IMMORTAL && damage >= target.health) {
                int floored = target.health - 1;
                if (floored < 0) {
                    floored = 0;
                }
    ...
    default void KillMobj(mobj_t source, mobj_t target) {
        ...
        if (target.health < -target.info.spawnhealth && target.info.hasXDeathState()) {
            ...
            target.SetMobjStateNum(target.info.deathStateNum());
        }
