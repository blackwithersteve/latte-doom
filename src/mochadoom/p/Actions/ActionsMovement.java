...
public interface ActionsMovement extends ActionsPathTraverse {
    ...
    default void XYMovement(mobj_t mo) {
        ...
        if (mo.momx > -STOPSPEED && mo.momx < STOPSPEED && mo.momy > -STOPSPEED && mo.momy < STOPSPEED
            ...
            // These are exactly T_Friction's per-tic assignment conditions.
            int friction = FRICTION;
            if (mo.player != null && mo.subsector != null) {
                final rr.sector_t fsec = mo.subsector.sector;
                if (fsec.boomFriction != rr.sector_t.ORIG_FRICTION
                    && (fsec.special & rr.sector_t.FRICTION_MASK) != 0
                    && mo.z <= fsec.floorheight
                    && !eval(mo.flags & (MF_NOGRAVITY | MF_NOCLIP))) {
                    friction = fsec.boomFriction;
                }
            }
