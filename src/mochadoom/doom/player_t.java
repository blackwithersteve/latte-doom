...
public class player_t /*extends mobj_t */ implements Cloneable, IReadableDoomObject, IPackableDoomObject {
    ...
    private int boomMoveFactor() {
        ...
        if (sec == null || sec.boomFriction == sector_t.ORIG_FRICTION
            || (sec.special & sector_t.FRICTION_MASK) == 0
            || mo.z > sec.floorheight
            || (mo.flags & (MF_NOGRAVITY | MF_NOCLIP)) != 0) {
            return PLAYERTHRUST;
        }
        int movefactor = sec.boomMoveFactor;
        if (sec.boomFriction < sector_t.ORIG_FRICTION) { // sludge: momentum eases the crawl
            final int momentum = p.MapUtils.AproxDistance(mo.momx, mo.momy);
    ...
    public void MovePlayer() {
        ...
        if (cmd.sidemove != 0 && onground) {
            Thrust((mo.angle - ANG90) & BITS32, cmd.sidemove * boomMoveFactor());
        }
    ...
    protected void PlayerInSpecialSector() {
        ...
        if ((sector.special & 0xFFE0) != 0) {
            switch ((sector.special >> 5) & 3) {
                case 1: // 5 units per 32 tics
                    if (powers[pw_ironfeet] == 0 && !flags(DOOM.leveltime, 0x1f)) {
                ...
                case 2: // 10 units
                    ...
                    break;
                case 3: // 20 units, the super-hellslime suit leak
                    if ((!eval(powers[pw_ironfeet]) || (DOOM.random.P_Random() < 5))
                        && !flags(DOOM.leveltime, 0x1f)) {
            ...
            if ((sector.special & 0x80) != 0) { // generalized secret
                secretcount++;
                sector.special &= ~0x80;
            }
            return;
...
//
    ...
    public void PlayerReborn() {
        ...
        ammo[ammotype_t.am_clip.ordinal()] = deh.DehMisc.initialBullets;
        lookdir = 0; // From Heretic
